...
int main(int argc, char const *argv[])
{
	...
	char autre_chemin[] = "/home/simon/hello.txt";
	...
	creer_fichier(autre_chemin,&disque);

	printf("Contenu du bloc 7 : %s\n", disque.bloc[7].donnees);

	printf("beep\n");
	inode = inode_via_chemin(autre_chemin_2,&disque);
	printf("via chemin : %d\n", inode);


	ecrire_fichier(inode,nom,&disque);

	ajouter_fichier(inode,texte,&disque);
